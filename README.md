...
SecureSnap is a web application I built to explore how common
cybersecurity practices can be applied to an online stock photo
e-commerce platform.

The basic idea is simple: users can create an account, log in,
browse stock photos, purchase photos, and download the photos they
have purchased.

Along with these basic features, I focused on adding security
mechanisms that protect user accounts and digital content from
...
An online platform that sells digital content needs to protect more
than just usernames and passwords. It also needs to make sure that
users cannot access content they have not purchased and that common
web attacks are handled properly.

For this project, I wanted to understand how these security concepts
work in an actual web application instead of only studying them
theoretically.
...
Passwords are not stored directly in the database. Instead, they are
...
Flask-Login is used to manage user sessions and protect pages that
...
The application also checks whether the user has already purchased
the same photograph to prevent duplicate purchases.
...
Before allowing a download, SecureSnap checks whether the current
user has actually purchased that photograph.

If they have not purchased it, the application returns a `403
Forbidden` response.
...
Important pages such as the photo store and photo details require
...
This prevents users who are not logged in from directly accessing
protected pages.
...
SecureSnap checks the user's purchase record before allowing a photo
to be downloaded.
...
4. CSRF Protection
Cross-Site Request Forgery (CSRF) protection is enabled using
Flask-WTF.
POST forms contain CSRF tokens so that requests cannot simply be
submitted from another website without the required security token.
...
The application uses SQLAlchemy for database operations instead of
building SQL queries directly from user input.
For example:
...
Jinja2's default HTML escaping also helps prevent user-supplied
HTML from being interpreted as executable content when rendered
through the templates.

Result: PASS ✅

9. Secure Download Authorization
The download route performs an authorization check before returning
the image.

The application checks:

Is the user logged in?
Does the requested photo exist?
Has the current user purchased the photo?
...
This provides a basic way of monitoring security-related activity
during the application's operation.

11. Security Testing

After implementing the security features, I tested the application
against several common scenarios.
...
I logged into the application and attempted to access the download
endpoint for a photo that had not been purchased.
...
The application redirected me to the login page instead of showing
the photo store.
...
I intentionally entered an incorrect password multiple times on the
login page.

After five attempts within the configured time period, the
...
For local development, the application has a fallback development
key so that it can run without additional configuration.

For a real deployment, a strong secret key should be provided through
an environment variable instead of keeping it in the source code.

⚠️ Current Limitations

SecureSnap is a student/portfolio cybersecurity project rather than
a production e-commerce system.
...
Through this project, I got practical experience with how different
security mechanisms work together inside a web application.
...
One of the main things I learned from the project is that simply
having a login system is not enough to make a web application secure.
Different layers of protection are needed for different types of
attacks and unauthorized access.

👩‍💻 Project

Project Name: SecureSnap
